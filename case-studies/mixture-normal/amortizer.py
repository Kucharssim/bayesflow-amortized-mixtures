...
global_summary_net = DeepSet(summary_dim=20,
                             num_dense_s1=3, num_dense_s2=3, num_dense_s3=3, num_equiv=3)
classification_net = Sequential([Dense(32, activation='relu') for _ in range(8)])
classification_net.add(Dense(3))


amortizer = AmortizedPosteriorMixture(
    amortized_posterior=AmortizedPosterior(
        inference_net=InvertibleNetwork(num_params=5, num_coupling_layers=12, coupling_design="spline"),
